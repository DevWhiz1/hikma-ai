  ...
  return (
    <div className="bg-white rounded-lg shadow-lg p-6">
      <div className="mb-6">
        <div className="flex items-center justify-between mb-4">
          <div>
            ...
            <p className="text-gray-600">AI-powered scheduling to find optimal meeting times and broadcast them to your students.</p>
          </div>
                ...
                >
                  ...
                  <div className="mb-3">
                    ...
                    <div className="flex flex-wrap gap-1">
                      {template.timeSlots.map((time, index) => (
                        <span
                          key={index}
      ...
      {existingBroadcasts.length > 0 && (
        <div className="mb-6">
          <h3 className="text-lg font-semibold text-gray-900 mb-4">Existing Active Broadcasts</h3>
          <div className="bg-yellow-50 border border-yellow-200 rounded-lg p-4 mb-4">
            <div className="flex items-center">
              <ClockIcon className="h-5 w-5 text-yellow-600 mr-2" />
              <p className="text-sm text-yellow-800">
                <strong>Note:</strong> The following time slots are already posted and will not appear in optimal times to prevent conflicts.
              </p>
            </div>
          </div>
          <div className="space-y-2">
            {existingBroadcasts.slice(0, 3).map((broadcast, index) => (
              <div key={index} className="bg-gray-50 border border-gray-200 rounded-lg p-3">
                <div className="flex items-center justify-between">
                  <div>
                    <p className="font-medium text-gray-900">{broadcast.title}</p>
                    <p className="text-sm text-gray-600">
                      {broadcast.meetingTimes.length} time slot(s) • Expires: {new Date(broadcast.expiresAt).toLocaleDateString()}
                    </p>
                  </div>
            ...
            {existingBroadcasts.length > 3 && (
              <p className="text-sm text-gray-500 text-center">
                +{existingBroadcasts.length - 3} more active broadcasts
              </p>
            )}
          </div>
        </div>
