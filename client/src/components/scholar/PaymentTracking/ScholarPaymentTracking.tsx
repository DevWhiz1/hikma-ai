...
interface ScholarPaymentTrackingProps {
  onPaymentSelect?: (payment: Payment) => void;
}

const ScholarPaymentTracking: React.FC<ScholarPaymentTrackingProps> = ({ onPaymentSelect }) => {
  ...
  const loadPayments = async () => {
    try {
      setLoading(true);
      const response = await paymentService.getScholarPayments({
        page: currentPage,
        limit: 10,
    ...
    <div className="min-h-screen bg-gradient-to-br from-emerald-50 via-white to-teal-50 dark:from-gray-900 dark:via-gray-800 dark:to-emerald-900/20 p-6">
      <div className="max-w-7xl mx-auto">
        ...
        <div className="mb-8">
          <div className="flex items-center justify-between mb-4">
            <div>
              ...
              <p className="text-gray-600 dark:text-gray-400 mt-2">Track your earnings, payments, and student transactions</p>
            </div>
            <button
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6 mb-8">
          <div className="bg-white dark:bg-gray-800 rounded-lg shadow p-6">
            <div className="flex items-center">
              <TrophyIcon className="h-8 w-8 text-emerald-600 mr-3" />
              <div>
                <p className="text-sm text-gray-600 dark:text-gray-400">Total Earnings</p>
                <p className="text-2xl font-bold text-gray-900 dark:text-white">{formatCurrency(analytics?.totalEarnings || 0)}</p>
              </div>
            </div>
          </div>
          <div className="bg-white dark:bg-gray-800 rounded-lg shadow p-6">
          ...
          <div className="bg-white dark:bg-gray-800 rounded-lg shadow p-6">
            <div className="flex items-center">
              <UsersIcon className="h-8 w-8 text-teal-600 mr-3" />
              <div>
                <p className="text-sm text-gray-600 dark:text-gray-400">Active Students</p>
                <p className="text-2xl font-bold text-gray-900 dark:text-white">{analytics?.topStudents || 0}</p>
              </div>
            </div>
          </div>
        </div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-6 mb-8">
          <div className="bg-white dark:bg-gray-800 rounded-lg shadow p-6">
            <div className="flex items-center">
              <ArrowUpIcon className="h-8 w-8 text-emerald-600 mr-3" />
              <div>
                <p className="text-sm text-gray-600 dark:text-gray-400">Monthly Earnings</p>
                <p className="text-xl font-bold text-gray-900 dark:text-white">{formatCurrency(analytics?.monthlyEarnings || 0)}</p>
              </div>
            </div>
          </div>
          <div className="bg-white dark:bg-gray-800 rounded-lg shadow p-6">
            <div className="flex items-center">
              <StarIcon className="h-8 w-8 text-teal-600 mr-3" />
              <div>
                <p className="text-sm text-gray-600 dark:text-gray-400">Avg Session Value</p>
                <p className="text-xl font-bold text-gray-900 dark:text-white">{formatCurrency(analytics?.averageSessionValue || 0)}</p>
              </div>
            </div>
          </div>
          <div className="bg-white dark:bg-gray-800 rounded-lg shadow p-6">
            <div className="flex items-center">
              <ClockIconSolid className="h-8 w-8 text-lime-600 mr-3" />
              <div>
                <p className="text-sm text-gray-600 dark:text-gray-400">Pending Payments</p>
                <p className="text-xl font-bold text-gray-900 dark:text-white">{analytics?.pendingTransactions || 0}</p>
              </div>
            </div>
          </div>
        </div>
              ...
              <div className="p-6 space-y-4">
                ...
                <div>
                  ...
                  <div className="flex items-center mt-2">
                    <div className="h-10 w-10 rounded-full bg-emerald-100 dark:bg-emerald-900/50 flex items-center justify-center">
                      <span className="text-sm font-medium text-emerald-600 dark:text-emerald-400">
                        {selectedPayment.user?.name?.charAt(0) || 'S'}
                      </span>
                    </div>
                    <div className="ml-4">
                      <div className="text-sm font-medium text-gray-900 dark:text-white">
...
export default ScholarPaymentTracking;
