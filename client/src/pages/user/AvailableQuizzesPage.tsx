...
const AvailableQuizzesPage: React.FC = () => {
  ...
  return (
    <div className="p-6">
      <h1 className="text-2xl font-bold text-emerald-700 dark:text-emerald-300 mb-4">Quizzes</h1>
      {loading ? (
        <p>Loading...</p>
      ) : (
          ...
          {items.map(item => (
            <div key={item._id} className="rounded border p-4 bg-white dark:bg-gray-900">
              <div className="flex items-center justify-between">
                ...
                <Link to={`/assignments/${item._id}/take`} className="px-3 py-2 text-sm rounded bg-emerald-600 text-white hover:bg-emerald-700">Start Quiz</Link>
              </div>
              {item.description && <p className="mt-2 text-sm text-gray-700 dark:text-gray-300">{item.description}</p>}
            </div>
          ))}
          {items.length === 0 && <div className="text-gray-600 dark:text-gray-400">No quizzes available.</div>}
        </div>
      )}
    </div>
  );
