...
// Simple endpoint to run cleanup
app.get('/cleanup', async (req, res) => {
  try {
    const result = await cleanupDuplicateChats();
    res.json({
      success: true,
      message: 'Duplicate chat cleanup completed successfully',
      duplicatesRemoved: result.duplicatesRemoved,
      orphanedSessionsRemoved: result.orphanedSessionsRemoved
    });
  } catch (error) {
    res.status(500).json({
      success: false,
